...
def get_tabs():
    ...
    # Optimized query to get unread counts for all tabs in one go
    # Group by Feed.tab_id to get counts per tab
    unread_counts_query = (db.session.query(Feed.tab_id, func.count(
        FeedItem.id)).select_from(FeedItem).join(Feed).filter(
            FeedItem.is_read == False).group_by(Feed.tab_id).all())
