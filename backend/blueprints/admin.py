...
def toggle_admin(user_id):
    if current_user.id == user_id:
        return (
            jsonify({"error": "Cannot toggle admin status for yourself"}),
            400,
        )
...
def export_db():
    ...
    if not db_uri.startswith("sqlite:///"):
        return (
            jsonify({"error": "Database export only supported for SQLite"}),
            400,
        )
    ...
    if not db_path.startswith(project_root) or not os.path.exists(db_path):
        return (
            jsonify({"error": "Database file not found or access denied"}),
            404,
        )
