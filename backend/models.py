...
class FeedItem(db.Model):
    ...
    def to_iso_z_string(dt_val: datetime.datetime | None) -> str | None:
        ...
        Optimization: The FeedItem validator enforces that datetimes from the database
        are naive UTC objects. We can bypass expensive timezone conversions and string
        replacements by directly appending 'Z' to the isoformat() output for naive datetimes.
        """
        if dt_val is None:
            return None
        ...
        if dt_val.tzinfo is None:
            # Naive datetime from DB (assumed UTC), optimize serialization
