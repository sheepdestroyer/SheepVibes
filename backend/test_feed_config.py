...
def test_get_max_concurrent_fetches_default():
    ...
    with patch("os.cpu_count", return_value=2), patch.dict(os.environ, {},
                                                           clear=True):
...
def test_get_max_concurrent_fetches_default_cap():
    ...
    with patch("os.cpu_count", return_value=10), patch.dict(os.environ, {},
                                                            clear=True):
...
def test_explicit_worker_limit_respects_cap():
    """Verify that an explicit environment variable that exceeds the cap is correctly restricted to WORKER_FETCH_CAP."""
