...
def safe_drop_constraint(table_name, constraint_name, type_="unique", **kwargs):
    ...
    if constraint_name is None:
        logger.warning(f"Attempted to safe_drop_constraint with None name on table {table_name}. Skipping check, letting Alembic handle it (likely will fail if not handled by batch).")
        ...
        with op.batch_alter_table(table_name, schema=kwargs.get("schema")) as batch_op:
    ...
    if constraint_exists(table_name, constraint_name, type_):
        ...
        logger.info(f"Constraint {constraint_name} not found on {table_name}, skipping drop.")
