...
from .models import db, Tab, Feed, FeedItem, User # Added User
from .auth import auth_bp, bcrypt as bcrypt_auth, token_auth # Added for auth
...
app = Flask(__name__)
app.config['SECRET_KEY'] = os.environ.get('FLASK_SECRET_KEY', 'your-super-secret-key-fallback') # Added for JWT and session

# Initialize Bcrypt with app
bcrypt_auth.init_app(app)
...
if os.environ.get('PYTEST_RUNNING') == 'true':
    logger.info("PYTEST_RUNNING detected: Using in-memory SQLite database.")
    app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///:memory:'
    app.config['CACHE_TYPE'] = 'NullCache' # Use NullCache for tests
...
# --- Cache Configuration ---
# Overridden by PYTEST_RUNNING check above for tests
if not app.config.get('TESTING'): # Only use RedisCache if not in testing mode (where NullCache is set)
    app.config["CACHE_TYPE"] = "RedisCache"
    app.config["CACHE_REDIS_URL"] = os.environ.get("CACHE_REDIS_URL", "redis://localhost:6379/0")
else: # Ensure CACHE_TYPE is set if PYTEST_RUNNING was true but CACHE_TYPE wasn't explicitly set there
    app.config.setdefault('CACHE_TYPE', 'NullCache')


app.config.setdefault('CACHE_DEFAULT_TIMEOUT', 300) # 5 minutes default timeout
...
def get_tabs():
    ...
    # Example of getting current user, if needed:
    # current_user = token_auth.current_user()
    # logger.info(f"Tabs requested by user: {current_user.username if current_user else 'Unknown'}")
