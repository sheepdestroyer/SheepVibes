...
def _process_opml_outlines_recursive(
    ...
    for outline_element in outline_elements:
        ...
        if xml_url:  # It's a feed
            ...
            ):  # child_outlines is already checked by the elif condition
            ...
            if (
                not xml_url
            ):  # If it's not a feed (it might be an empty folder or an invalid item)
                skipped_count_wrapper[0] += 1
        ...
        jsonify(
            {
                ...
                "skipped_count": skipped_final_count,
                # This might be the ID of a tab that was just deleted if it was the empty "Imported Feeds"
                "tab_id": top_level_target_tab_id,
...
def get_tabs():
    ...
    # Optimized unread count retrieval:
    # 1. Query for unread counts grouped by tab_id.
    # 2. Join Feed and FeedItem tables.
    # 3. Filter for unread items.
    unread_counts_query = (
    ...
    # Convert query result to a dictionary mapping tab_id to count
    unread_counts = dict(unread_counts_query)

    # Pass the pre-calculated unread count to to_dict to avoid N+1 queries
    return jsonify(
