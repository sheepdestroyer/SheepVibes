...
check_for_no_remaining_issues() {
    ...
    if jq -e 'any(.[] | .body | gsub("\r"; ""); test("^(No remaining issues|All issues resolved|All fixed|No issues remaining|Looks good to merge|Ready to merge|LGTM)"; "im"))' "$comments_file" > /dev/null; then
