...
echo "   The 'sheepvibespod.pod' file (and other *.volume files) already include an '[Install]' section."
echo "   This means that after 'systemctl --user daemon-reload' is run, systemd is aware that these"
echo "   services are 'WantedBy=default.target' (or other targets)."
echo "   The pod should start automatically when you log in. No separate 'enable' command for the"
echo "   generated '${POD_SERVICE_NAME}' is needed or will work directly."
echo "   To confirm, you can log out and log back in, then check:"
echo "   systemctl --user status ${POD_SERVICE_NAME}"
echo ""
