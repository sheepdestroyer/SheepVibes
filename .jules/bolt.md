...
## 2026-02-16 - Optimized feed item updates
**Learning:** Updating feeds triggered a query fetching *all* existing items for that feed to check for duplicates, which is O(N) where N is total items.
**Action:** Pre-calculate potential GUIDs and Links from the incoming feed entries (O(M) where M is new items) and use `WHERE guid IN (...)` to fetch only relevant existing items.
