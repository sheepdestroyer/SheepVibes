...
## 2026-03-05 - Optimized to_iso_z_string serialization
**Learning:** `FeedItem.to_iso_z_string()` uses expensive tzinfo replacement for timezone naive datetimes that are already assumed to be UTC by database validators, which significantly slows down serialization performance for APIs returning many feed items.
**Action:** Simply append "Z" directly to the `isoformat()` string for naive datetimes instead of replacing tzinfo and string manipulation. This yields a ~3x performance speedup during feed serialization without any functionality changes.
