...
**Action:** Use a subquery with `order_by(DESC).offset(N)` inside a `DELETE WHERE id IN (...)` to remove excess items efficiently without loading IDs into application memory.
