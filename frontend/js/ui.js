...
export function renderTabs(tabs, activeTabId, callbacks) {
    ...
    if (!tabs || tabs.length === 0) {
        ...
        emptySpan.textContent = 'No tabs found.';
        tabsContainer.replaceChildren(emptySpan);
        renameTabButton.disabled = true;
        deleteTabButton.disabled = true;
        const emptyP = document.createElement('p');
        emptyP.textContent = 'Create a tab to get started!';
        feedGrid.replaceChildren(emptyP);
