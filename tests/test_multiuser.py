...
def test_register_and_login(client):
    # Register
    res = client.post("/api/auth/register", json={"username": "testuser", "password": "password123"})
...
def test_multiuser_isolation(client):
    # Create two users
    client.post("/api/auth/register", json={"username": "u1", "password": "p1"})
...
def test_admin_access(client):
    # First user is admin
    client.post("/api/auth/register", json={"username": "admin", "password": "p1"})
    client.post("/api/auth/login", json={"username": "admin", "password": "p1"})
    ...
    client.post("/api/auth/logout")
    client.post("/api/auth/register", json={"username": "user", "password": "p2"})
