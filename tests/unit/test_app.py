...
def test_add_feed_invalid_url_scheme(client):
    """Test POST /api/feeds with an invalid URL scheme."""
    response = client.post("/api/feeds",
                           json={
...
def test_update_feed_invalid_url_scheme(client, setup_tabs_and_feeds):
    """Test PUT /api/feeds/<feed_id> with an invalid URL scheme."""
    feed_id = setup_tabs_and_feeds["feed1_id"]
    response = client.put(f"/api/feeds/{feed_id}", json={"url": "file:///etc/passwd"})
    assert response.status_code == 400
    assert "Invalid feed URL scheme" in response.get_json()["error"]
