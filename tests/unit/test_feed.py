...
from backend import feed_service  # Import feed_service relatively
from backend.app import app  # Import app for context
from backend.models import Feed, FeedItem, Tab, db  # <--- Added FeedItem import
...
def test_per_feed_guid_uniqueness_and_null_guid_behavior(db_setup, mocker):  # pylint: disable=unused-argument
    ...
    tab = Tab(user_id=1, name="Mixed", order=1)
    db.session.add(tab)
    db.session.commit()

    feed1_obj = Feed(name="Feed1", url="http://dummy.feed1/rss")
    feed2_obj = Feed(name="Feed2", url="http://dummy.feed2/rss")
...
def test_update_feed_last_updated_time(db_setup, mocker, mock_dns):  # pylint: disable=unused-argument
    ...
    tab = Tab(user_id=1, name="Timestamps", order=1)
    db.session.add(tab)
    db.session.commit()
    ...
    success, new_items, _ = feed_service.fetch_and_update_feed(feed_obj.id)
...
def test_update_all_feeds_basic_run(db_setup, mocker, mock_dns):  # pylint: disable=unused-argument
    ...
    # Add feeds
    tab = Tab(user_id=1, name="TestTab", order=0)
    db.session.add(tab)
    db.session.commit()
    feed1 = Feed(name="FeedA_init", url="http://feedA.url")
    feed2 = Feed(name="FeedB_init", url="http://feedB.url")
...
def test_integrity_error_fallback_to_individual_commits(db_setup, mocker):  # pylint: disable=unused-argument
    ...
    tab = Tab(user_id=1, name="Test Tab Fallback", order=0)
    db.session.add(tab)
    db.session.commit()
...
def test_feed_item_eviction_on_limit_exceeded(db_setup, mocker):  # pylint: disable=unused-argument, too-many-locals
    ...
    tab = Tab(user_id=1, name="Eviction Test Tab", order=0)
    db.session.add(tab)
    db.session.commit()
