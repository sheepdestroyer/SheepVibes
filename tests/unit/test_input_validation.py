...
def test_feed_url_length_validation_create(client):
    ...
    response = client.post("/api/feeds", json={"url": long_url, "tab_id": tab_id})
    assert response.status_code == 400
...
def test_feed_url_length_validation_update(client):
    ...
    response = client.post("/api/feeds", json={"url": "http://example.com/valid", "tab_id": tab_id})
    assert response.status_code == 201
...
def test_feed_name_length_validation_update(client):
    ...
    response = client.post("/api/feeds", json={"url": "http://example.com/valid2", "tab_id": tab_id})
    assert response.status_code == 201
    feed_id = response.json["id"]

    long_name = "a" * 201
    valid_url = "http://example.com/feed.xml"
    response = client.put(f"/api/feeds/{feed_id}", json={"url": valid_url, "name": long_name})
    assert response.status_code == 400
