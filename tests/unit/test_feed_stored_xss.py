...
from backend.models import Tab, Feed, db
...
def test_add_feed_stored_xss(client):
    ...
    response = client.post(
        "/api/feeds",
        json={"url": "javascript:alert(1)", "tab_id": tab_id}
    )
...
def test_update_feed_url_stored_xss(client):
    ...
    with app.app_context():
        ...
        db.session.commit()
        tab_id = tab.id
        feed = Feed(tab_id=tab_id, name="Valid Feed", url="https://example.com/feed.xml")
        db.session.add(feed)
        db.session.commit()
        feed_id = feed.id
    ...
    response = client.put(
        f"/api/feeds/{feed_id}",
        json={"url": "javascript:alert(2)", "name": "New Name"}
    )
    assert response.status_code == 400
    assert "Invalid" in response.get_json()["error"]
