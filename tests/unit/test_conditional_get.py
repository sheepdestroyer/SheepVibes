...
def test_fetch_feed_handles_304(mocker):
    ...
    mock_http_error = urllib.error.HTTPError(url="http://example.com/feed",
                                             code=304,
                                             msg="Not Modified",
                                             hdrs={},
                                             fp=None)
...
def test_fetch_feed_captures_response_headers(mocker):
    ...
    mocker.patch(
        "backend.feed_service.feedparser.parse",
        return_value=feedparser.FeedParserDict(entries=[], bozo=0),
    )
