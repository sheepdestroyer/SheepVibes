...
from backend.models import Feed, Subscription, Tab, User, db
...
def test_import_nested_opml(client, mocker):
    ...
    with app.app_context():
        ...
        tech_tab = Tab.query.filter_by(
            user_id=user.id, name="Tech Folder").first()
        assert tech_tab is not None
        sub_tech_tab = Tab.query.filter_by(
            user_id=user.id, name="Sub Tech Folder"
        ).first()
        ...
        assert (
            Subscription.query.filter_by(
                user_id=user.id, feed_id=hn_feed.id, tab_id=tech_tab.id
            ).first()
            is not None
        )
...
<opml version="2.0">
  <body>
    ...
    with app.app_context():
        user = User.query.first()
        skipped_tab = Tab.query.filter_by(
            user_id=user.id, name="Skipped Folder"
        ).first()
...
def test_opml_import_skips_duplicate_feed_urls(client, mocker):
    with app.app_context():
        ...
        db.session.flush()
        existing_feed = Feed(
            name="Existing Feed", url="https://example.com/existing.xml"
        )
        db.session.add(existing_feed)
        db.session.flush()
        sub = Subscription(
            user_id=user.id, tab_id=base_tab.id, feed_id=existing_feed.id
        )
        db.session.add(sub)
        db.session.commit()
    opml_content = b"""<?xml version="1.0" encoding="UTF-8"?>
