...
def close_prs(dry_run=False):
        ...
        if state == "closed":
            ...
            if not dry_run:
                ...
                if ok:
                    ...
                    print(
                        f"  -> Branch '{head_ref}' delete failed or already gone")
    ...
    print("RESULTS:")
    print(
        f"  Already closed: {len(results['already_closed'])} {results['already_closed']}"
    )
    ...
    print(
        f"  Branch failed:  {len(results['branch_failed'])} {results['branch_failed']}"
    )
...
if __name__ == "__main__":
    ...
    mode = (
        len(sys.argv) > 1
        and sys.argv[1] in ("close", "check", "dry-run")
        and sys.argv[1]
        or "check"
    )
