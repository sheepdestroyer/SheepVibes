...
1.  **Service Containerization**: The tests require a running Valkey service for caching checks. In the GitHub Actions workflow, this is accomplished using the `services` block to start a `docker.io/valkey/valkey:9.1-alpine` container. Your environment must provide a Valkey container accessible to the test runner.
