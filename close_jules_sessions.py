...
def api_get(path):
    """GET from Jules API."""
    cmd = ["curl", "-s", "-H", f"x-goog-api-key: {TOKEN}", f"{BASE}{path}"]
...
def close_session(
    session_id, reason="Duplicate PRs have been cleaned up. Closing session."
):
...
def main():
    ...
    mode = (
        len(sys.argv) > 1
        and sys.argv[1] in ("close", "list", "dry-run")
        and sys.argv[1]
        or "list"
    )
