...
# Usage: ./merge_prs_v2.sh PR_NUMBER
...
if git diff --name-only --diff-filter=U 2>/dev/null | grep -q .; then
    ...
    for f in $(git diff --name-only --diff-filter=U 2>/dev/null | grep "^\.jules/"); do
...
# Remove noise files that shouldn't be in the repo
for f in backend/*.bak backend/*.bak2 backend/*.before_edit backend/cache_utils.py.bak \
         close_jules_sessions.py close_prs.py merge_prs.sh merge_prs_v2.sh update_prs.py \
         *.py.patch backend/*.py.patch backend/blueprints/*.py.patch; do
    [ -f "$f" ] && git rm -f "$f" 2>/dev/null || true
done
git commit -m "Remove noise files" 2>/dev/null || true
